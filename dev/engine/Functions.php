...
{
    return DataService::get_data($name, $sub_dir);
}

function render_component(string $component_name, array $data = []): string
{
    return ComponentRenderer::render($component_name, $data);
}

function get_component_asset_tags(): string
{
    return ComponentRenderer::get_component_asset_tags();
}

function seo_implicit_fields(): array
{
    $fields = [];
    $global = DataService::get_global_settings();
...
function seo_add_in_content(array $seo_data, string $existing_html = ''): string
{
    ...
    $slug = $seo_data['slug'] ?? '';
    if (!empty($slug)) {
        $ext = EngineConfig::getPageSlugExtension();
...
function get_seo_from_current_page_data(string $filename): ?array
{
    ...
    foreach (DataService::get_valid_post_files() as $pf) {
...
function svg_get(string $icon): string
{
    ...
    if ($icons_data === null) {
        $data_file = EngineConfig::get('data_dir') . '/data_icons.json';
