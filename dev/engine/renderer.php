...
class renderer
{
    ...
    {
        $results = [];
        $dist_dir = engine_config::get('dist_dir');

        if (!is_dir($dist_dir)) {
            mkdir($dist_dir, 0755, true);
        }

        $valid_posts = data_service::get_valid_post_files();

        foreach ($valid_posts as $post_file) {
            $post_name = $post_file['name'];
            $posts = data_service::get_all_posts_from_file($post_name);
    ...
    {
        if ($output_dir === null) {
            $output_dir = engine_config::get('dist_dir');
        }
    ...
    public static function render_post_html(string $post_name, array $post): string
    {
        ...
        foreach ($content_sections as $section) {
            ...
            switch ($post_name) {
                ...
                case 'workstations':
                    ...
                    $merged_data['post_type'] = $post_name;
                    $merged_data['colors'] = $post['settings']['appearance']['colors'] ?? [];
                    break;
                default:
            ...
            $section_html = component_renderer::render($component_name, $merged_data);
        ...
        return component_renderer::render('page_structure', $page_data);
    }
    ...
    {
        $config_path = engine_config::get('dev_dir') . '/compile/scss/scss-config.json';
    ...
    {
        require_once engine_config::get('dev_dir') . '/compile/scss/compile.php';
