...
class ComponentRenderer
{
    ...
    public static function render(string $component_name, array $data = []): string
    {
        ...
        if ($config === null) {
            return '<!-- Component not found: ' . htmlspecialchars($component_name) . ' -->';
        }
        ...
        if (empty($php_files)) {
            return '<!-- No PHP files for component: ' . htmlspecialchars($component_name) . ' -->';
        }
        ...
        if (!isset(self::$loaded_classes[$class_name])) {
            ...
            if (!$loaded || !class_exists($class_name, false)) {
                return '<!-- Component class not found: ' . htmlspecialchars($class_name) . ' -->';
            }
        ...
        if (!class_exists($class_name, false)) {
            return '<!-- Component class not loaded: ' . htmlspecialchars($class_name) . ' -->';
        }
        ...
        return '<!-- Component has no render method: ' . htmlspecialchars($class_name) . ' -->';
    }
    ...
    public static function get_component_asset_tags(): string
    {
        ...
        $url_path = EngineConfig::getUrlPath();
        $component_names = DataService::get_all_component_names();
        ...
        foreach ($component_names as $name) {
            ...
            // JS files are bundled into components.js — no individual script tags needed
        }
