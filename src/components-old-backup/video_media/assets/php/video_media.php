...
class VideoMedia
{
    ...
    protected static function render_video_folder(string $postType, string $mediaPath, array $postData): string
    {
        ...
        $videoPath = $urlPath . 'src/content/vid/' . $postType . '/' . $mediaPath . '/';
        $logoPath = $urlPath . 'src/content/img/' . $postType . '/' . $mediaPath . '/' . ($postData['logo'] ?? '');
        $thumbnailBg = $urlPath . 'src/content/img/thumbnails/workpreview-overlay-thumbnail.webp';
    ...
    protected static function render_strict_videos(array $videos): string
    {
        ...
        foreach ($videos as $video) {
            $html .= ComponentRenderer::render_component('video', [
                'src' => $urlPath . 'src/content/vid/' . ($video['video_path'] ?? ''),
                'thumbnail' => $urlPath . 'src/content/img/' . ($video['video_thumbnail_path'] ?? ''),
            ]);
        }
        return $html;
    }
