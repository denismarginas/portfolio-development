...
class DevicesPostItemMedia
{
    public static function render(array $data): string
    {
        ...
        $device_layout_img_path = $GLOBALS['urlPath'] . "src/content/img/" . "design-elements" . "/";
        ...
        if (isset($post_data["post_type"]) && isset($post_data["media_path"])) {
            $media_image_path = $GLOBALS['urlPath'] . "src/content/img/" . $post_data["post_type"] . "/" . $post_data["media_path"] . "/media/";
            $abs_path = realpath(__DIR__ . "/../../../../../") . "/" . $media_image_path;

            if (is_dir($abs_path)) {
                $get_media_image = Helpers::get_images_in_folder($abs_path);
                $dir_image_path = "";

                if (count($get_media_image) === 0) {
                    $dirs = Helpers::get_directories_in_folder($abs_path);
                    foreach ($dirs as $dir) {
                        $get_media_image = Helpers::get_images_in_folder($abs_path . $dir . "/");
                        if (count($get_media_image) > 0) {
                            $dir_image_path = $dir . "/";
                            break;
                        }
            ...
            $media_video_path = $GLOBALS['urlPath'] . "src/content/vid/" . $post_data["post_type"] . "/" . $post_data["media_path"] . "/";
            $abs_vid_path = realpath(__DIR__ . "/../../../../../") . "/" . $media_video_path;

            if (is_dir($abs_vid_path)) {
                $get_media_video = Helpers::get_videos_in_folder($abs_vid_path);
                $dir_video_path = "";

                if (count($get_media_video) === 0) {
                    $dirs = Helpers::get_directories_in_folder($abs_vid_path);
                    foreach ($dirs as $dir) {
                        $get_media_video = Helpers::get_videos_in_folder($abs_vid_path . $dir . "/");
                        if (count($get_media_video) > 0) {
                            $dir_video_path = $dir . "/";
                            break;
                        }
