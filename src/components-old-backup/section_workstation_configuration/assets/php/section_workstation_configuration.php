...
class SectionWorkstationConfiguration
{
    public static function render(array $data = []): string
    {
        ...
        if (!empty($path) && !empty($img)) {
            $workstationImage .= render_image($GLOBALS['urlPath'] . "src/content/img/" . $path . "/" . $img);
        }
        ...
        foreach ($products as $key => $product) {
            ...
            $productCards .= render_image($GLOBALS['urlPath'] . "src/content/img/" . $path . "/" . $product["img_src"]);
            if (isset($product["img-src-box"])) {
                $productCards .= render_image($GLOBALS['urlPath'] . "src/content/img/" . $path . "/" . $product["img-src-box"]);
            }
            $productCards .= '</div>';
